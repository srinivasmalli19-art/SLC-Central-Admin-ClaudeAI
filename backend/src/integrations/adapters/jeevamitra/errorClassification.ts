...
// could reveal credential details). Identical logic to the Pasumithra
// adapter's classifier — duplicated rather than shared so each adapter
// stays self-contained (matches the existing codebase's per-adapter
// folder convention) and so this file's messages can name JeevaMitra
// specifically without a generic/parameterized abstraction that no other
// adapter currently needs.
//
// Practical note on test coverage (see docs/JEEVAMITRA-ADAPTER.md
// "Testing"): the Firestore emulator ignores credentials entirely, so it
// can never produce a genuine auth failure — the auth_error branch here is
...
export function classifyFirestoreError(err: unknown): ClassifiedError {
  ...
  if (ADC_NOT_CONFIGURED_PATTERNS.some((pattern) => pattern.test(rawMessage))) {
    return {
      ...
      message: "JeevaMitra integration is configured for Application Default Credentials, but none were found.",
    };
  }

  if (code !== undefined && AUTH_ERROR_CODES.has(code)) {
    return { status: "auth_error", message: "JeevaMitra rejected the configured credential." };
  }
  if (code !== undefined && UNAVAILABLE_CODES.has(code)) {
    return { status: "unavailable", message: "JeevaMitra's Firestore was unreachable." };
  }
  ...
  return { status: "unavailable", message: "Could not reach JeevaMitra's Firestore." };
}
