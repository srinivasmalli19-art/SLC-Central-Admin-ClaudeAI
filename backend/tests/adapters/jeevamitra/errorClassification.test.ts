...
import { classifyFirestoreError } from "../../../src/integrations/adapters/jeevamitra/errorClassification.js";

// The Firestore emulator ignores credentials entirely, so it can never
// produce a genuine Google auth rejection — see
// docs/JEEVAMITRA-ADAPTER.md "Testing". These synthetic error objects are
...
describe("classifyFirestoreError — never leaks credential material", () => {
  it("never includes a private-key-shaped value in its returned message, even if the raw error contained one", () => {
    ...
    const result = classifyFirestoreError(new Error(`invalid_grant: bad key: ${fakeSecret}`));
